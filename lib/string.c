...
/* Finds and returns the first occurrence of C in STRING, or a
   ...
   STRING. */
char *
strchr(const char *string, int c_) // string에서 c가 있는지 검사해서 해당 부분의 포인터 반환, 아니면 NULL반환
{
	char c = c_;
...
*/
char *
strtok_r(char *s, const char *delimiters, char **save_ptr) // 문자열 s 에서 구분자 delimiters를 기준으로 자르고, 문자 처음을 리턴하고 save_ptr에 델리미터 다음 포인터 저장
{
	char *token;
